...
exports.broadcastWith = void 0;
const rxjs_1 = require("rxjs");
function broadcastWith(type, subjectOrValue) {
    switch (type) {
        case "multicast":
            if (!(subjectOrValue instanceof rxjs_1.Subject)) {
                throw new Error(`Subject is required for ${type}`);
            }
            return (source$) => source$.pipe((0, rxjs_1.multicast)(subjectOrValue));
        case "publish":
            return (source$) => source$.pipe((0, rxjs_1.publish)());
        case "publishBehavior":
            return (source$) => source$.pipe((0, rxjs_1.publishBehavior)(subjectOrValue));
        case "publishReplay":
            return (source$) => source$.pipe((0, rxjs_1.publishReplay)(subjectOrValue));
        case "share":
            return (source$) => source$.pipe((0, rxjs_1.share)());
        default:
            throw new Error(`Unknown broadcast type: ${type}`);
    }
}
exports.broadcastWith = broadcastWith;
